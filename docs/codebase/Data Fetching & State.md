...
`src/lib/query-keys.ts` — central ==`QK` const== of string segments (LIST, DETAIL, MY, ALL, ME, SEARCH, SUMMARY, HISTORY, TRIP, ORGANIZER, MINE, BOOKING, TRIP_STATUS, ADMIN, ORGANIZERS, BOOKINGS, CASHBACK, REVIEWS, TRANSACTIONS, INVITES, TRIPS, REQUESTS, ACTIVE, POPULAR, PUBLIC, DOC_REVIEW, BY_USER, BY_TRIP, CONVERSATIONS, MESSAGES, UNREAD, UNREAD_COUNT, FLAGGED, SEAT_MAP, ORGANIZER_SEAT_MAP, SIGNATURE, ALL_PENDING, MY_REQUESTS, RESELLER, MAIN_LINKS, SUBLINKS, LEADS, RESOLVE, RESELLERS, ==PAYOUTS, PENDING==) plus typed `as const` factories:
...
| `adminKeys`                                                                       | stats / organizers* / bookings* / cashback* / invites / trips / reviews / docReviewDetail / travellers* / organizerDirectory* / **payoutsPending\*** / **payoutsHistory\*** |
...
- **Bookings**: `use-create-booking`, `use-cancel-booking`, `use-my-bookings`, `use-my-booking-summary`, `use-my-trip-booking-status`, `use-my-pending-requests`, `use-all-pending-requests`, `use-booking-contact` (`useSendBookingContactOtp(bookingId)`/`useVerifyBookingContactOtp(bookingId)`/`useUseAccountPhoneForBooking(bookingId)` — `POST /bookings/:id/contact/{send-otp,verify-otp,use-account-phone}`; the latter two invalidate `bookingKeys.all` on success. Booking-scoped, never reads/writes `useAuthStore` — contrast with `use-attach-phone`, which is account-scoped and correctly does touch the store)
- **Payments/Wallet**: `use-payments`, `use-sync-payment`, `use-verify-payment`, `use-wallet`
- **Auth**: `use-google-auth`, `use-firebase-phone-auth`, `use-email-otp`, `use-otp`, `use-logout`, `use-attach-phone` (`useSendAttachPhoneOtp`/`useVerifyAttachPhoneOtp` — `POST /auth/otp/attach/send`+`/verify`, the **authenticated** phone-attach pair, distinct from `use-otp`'s public login flow. On verify success, calls `useAuthStore.getState().updateUser({ phone, phoneVerified: true })` — ==never `setAuth`==, since the backend never issues tokens for this endpoint and the caller's existing session must stay untouched — then invalidates `profileKeys.me()`. Contrast with every login hook (`use-otp`, `use-email-otp`, `use-google-auth`), which correctly call `setAuth` because those *do* replace the session), `use-attach-email` (`useSendAttachEmailOtp`/`useVerifyAttachEmailOtp` — `POST /auth/otp/attach-email/send`+`/verify`, mirrors `use-attach-phone` exactly for email: `updateUser({ email, emailVerified: true })` on verify success, never `setAuth`, then invalidates `profileKeys.me()`), `use-redirect-if-authenticated` (`useRedirectIfAuthenticated({ requireOnboarded?, returnTo? })` — the shared "bounce an already-authenticated visitor away from this public login/signup page" effect, used by `login/email`, `login/email-otp`, `login/phone`, `signup`, and `signup/organizer/[token]`. **Must** route through `getPostAuthRoute`, never a raw `getHomeRoute`/hardcoded path — this effect re-fires on the next render after a submit handler calls `setAuth()`/`markOnboardingComplete()`, so a disagreement between the two redirect sources lets this later-firing effect silently override the handler's destination. This exact race shipped once across all 5 pages when each inlined its own copy with `getHomeRoute`; extracting one hook removes the duplication that let the bug recur. `getPostAuthRoute` no longer has a phone-verification branch — see [[Auth & Security#Frontend Guards]].)
- **Admin**: `use-admin-bookings`, `use-admin-cashback`, `use-admin-chat`, `use-admin-invites`, `use-admin-organizers`, `use-admin-reviews`, `use-admin-stats`, `use-admin-trips`, `use-admin-set-trip-visibility`, `use-admin-toggle-trip-bookings`, `use-admin-travellers` (traveller directory + detail), `use-admin-organizer-directory` (organizer directory + trips-created detail), `use-admin-payouts` (`useAdminPendingPayouts`/`useAdminPayoutHistory`/`useAdminReleasePayout` — organizer wallet-ledger payouts for the RazorpayX Payouts strategy, `/admin/payouts/*`; `useAdminPendingPayouts` takes `{ page, limit }` and both it and `useAdminPayoutHistory` return `{ data, pagination }` — the pending-payouts table owns its own `page` state and renders the shared `Pagination` component, same pattern as the wallet-activity table; the release mutation invalidates both `payoutsPending()` and `payoutsHistoryBase()` and toasts using the response's `status`/`releasedAmount`, since a requested amount can be capped by `insufficient_balance`. See [[Payments & Webhooks]] "Organizer earnings via Wallet ledger".)
