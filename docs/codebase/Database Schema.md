...
Prisma 6 + PostgreSQL. Schema: `apps/api/prisma/schema.prisma` (`url = DATABASE_URL`, `directUrl = DIRECT_URL`). 50 migrations in `apps/api/prisma/migrations/` (latest: `20260728050000_add_organizer_payout_attempt`); scaling notes in `apps/api/prisma/DB_SCALING_RUNBOOK.md`.
...
| PaymentType           | PAYMENT, REFUND, ESCROW_RELEASE, DEPOSIT_RELEASE, BALANCE_RELEASE, ==PAYOUT_RELEASE== (Cashfree deposit/balance payout split, see [[Payments & Webhooks]]; ESCROW_RELEASE remains the Razorpay Route SafePay path; PAYOUT_RELEASE is the RazorpayX Payouts strategy analogue of ESCROW_RELEASE — migration `20260728010000_add_razorpayx_payouts_enums_and_columns`) |
| PaymentStatus         | INITIATED, AUTHORIZED, CAPTURED, FAILED, REFUNDED, ==PROCESSING, REVERSED== (the last two are RazorpayX Payouts lifecycle only)                                                                   |
...
| VerificationCodeType  | EMAIL_VERIFY, PHONE_OTP, EMAIL_OTP, PASSWORD_RESET, BOOKING_CONTACT_OTP — booking-scoped contact verification, kept distinct from `PHONE_OTP` so the two flows never invalidate each other's in-flight code for the same phone number; ==ATTACH_PHONE_OTP, ATTACH_EMAIL_OTP== — authenticated "attach" flows (profile edit), kept distinct from the public login `PHONE_OTP`/`EMAIL_OTP` for the same reason |
...
| WalletTransactionType | REFUND, CASHBACK, BOOKING_DEDUCTION, ADMIN_CREDIT, ADMIN_DEBIT, PROMOTIONAL_CREDIT, EXPIRY, ==ORGANIZER_EARNING, ORGANIZER_EARNING_REVERSAL, ORGANIZER_PAYOUT, ORGANIZER_PAYOUT_REVERSED== (organizer earnings ledger, reusing the traveler-only Wallet/WalletTransaction system — migration `20260728020000_add_organizer_earning_wallet_transaction_types`; see [[Payments & Webhooks]] "Organizer earnings via Wallet ledger") |
| ==OrganizerPayoutAttemptStatus== | INITIATED, SUCCEEDED, FAILED — `OrganizerPayoutAttempt` ledger-before-gateway-call rows only (migration `20260728050000_add_organizer_payout_attempt`; see [[Payments & Webhooks]]) |
...
- **Trip** — `organizerId`, `destinationId`, `title`, `slug` *(unique)*, `tripType`, `description`, `itinerary (Json)`, `startDate`/`endDate`, `bookingDeadline?`, `pricePerPerson (Int)`, `earlyBirdPrice?`/`earlyBirdDeadline?`, `minGroupSize`/`maxGroupSize`, `currentBookings`, ==`trendingScore?` (Float, cron-recomputed)==, `version`, `inclusions`/`exclusions (Json)`, `cancellationPolicy`, `photos (String[])`, `itineraryDocUrl?`, `status`, `bookingMode`, `acceptingBookings` (+ `bookingsPausedReason`/`bookingsPausedBy` — note: field names are `bookingsPaused*`, not `paused*`), `isHidden` (+ `hiddenReason`/`hiddenBy`), `seatSelectionEnabled`, ==`commissionRate` (Decimal 5,2, default 10)== — frozen snapshot of `OrganizerProfile.commissionRate` taken at trip-creation time (`TripService.createTrip`, server-authoritative — never client-submitted); never re-snapshotted on `updateTrip`. Exists so a later admin edit to the organizer's live `commissionRate` (`PATCH /admin/organizers/:id/commission`) cannot retroactively change an already-published trip's economics — every downstream entitlement/payout calculation reads this frozen value (or, once a booking exists, the booking's own frozen copy — see **Booking** below), never `organizer.commissionRate` live. Heavy composite indexes for search/sort (incl. pg_trgm for review search per latest migration). `isHidden`/`hiddenReason`/`hiddenBy`/`bookingsPausedReason`/`bookingsPausedBy` were added to schema.prisma in commit `9c30628` without a migration (dev DBs got them via `prisma db push`); the missing migration `20260721195050_add_trip_hidden_and_bookings_paused_fields` was generated after the fact to bring production in sync.
...
- **Booking** — `bookingRef` *(unique)*, `tripId`, `userId`, `numTravelers`, `totalAmount`, `walletAmount`, `tripProtection`, `bookingStatus`, `expiresAt?`, `cancellationReason/At/ById`, `pickupPointId?`/`dropPointId?` → TripTransferPoint, `tripReminderSentAt?`, ==`sublinkId?` → ResellerSublink== (nullable — non-reseller bookings unaffected), ==`markupAmount` (Int, default 0)== — frozen snapshot (`markupPerPerson × numTravelers`) at booking time, never recomputed later. ==`commissionRate` (Decimal 5,2, default 10)== — frozen snapshot of `Trip.commissionRate` (itself frozen from the organizer's rate at trip-creation time) taken at booking-creation time (`BookingService.createBooking`); every downstream entitlement/payout calculation (capture-time `ORGANIZER_EARNING` wallet credit, `TripLifecycleService.resolveAndRelease`'s escrow release, `PaymentHistoryService`'s per-booking pending-payout figures) reads this column, never `trip.organizer.commissionRate` live — so an admin changing the organizer's rate after this booking was placed can never retroactively change what it pays out. Raw partial-unique: ==one active booking per (user, trip)==.
...
- ==**OrganizerPayoutAttempt**== — `organizerId` → OrganizerProfile, `idempotencyKey` *(unique, derived from this row's own `id` via `buildIdempotencyKey('ORG_WALLET_PAYOUT', id)`)*, `requestedAmount (Int, rupees)`, `status` (`OrganizerPayoutAttemptStatus`: INITIATED/SUCCEEDED/FAILED), `gatewayTransferId?`. Ledger-before-gateway-call row for `PayoutService.releaseOrganizerWalletPayout` (the admin-triggered organizer wallet-ledger payout) — written **inside** the distributed lock, **before** `razorpayxClient.createPayout` is called, mirroring the `PaymentTransaction` ledger-before-gateway-call pattern already used by `releaseBalance`/`releaseRazorpayXPayout`. Also backs the pre-flight "any recent INITIATED attempt for this organizer" check (index `[organizerId, status, createdAt]`) that guards against a slow gateway call outliving the release lock's TTL. Migration `20260728050000_add_organizer_payout_attempt`. See [[Payments & Webhooks]].
