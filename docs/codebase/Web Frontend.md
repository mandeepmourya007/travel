...
| `auth/` | Login/signup form building blocks — `phone-input-form.tsx` (`PhoneInputForm`, accepts an `onSubmit` override that replaces its default public `useSendOtp` call), `email-input-form.tsx` (`EmailInputForm`, same `onSubmit` override pattern, replaces its default public `useSendEmailOtp` call), `otp-verify-form.tsx` (`OtpVerifyForm`, 4/6-digit boxes + resend countdown, identifier-agnostic), `google-auth-section.tsx`, `onboarding-form.tsx`. `phone-verification-flow.tsx` (`PhoneVerificationFlow`) and `email-verification-flow.tsx` (`EmailVerificationFlow`) — a matched pair of reusable "attach + verify" wirings for the optional, account-level profile edit modal (**not** the mandatory gate — that has been retired): each owns its own `'phone'\|'otp'` / `'email'\|'otp'` step state, composing `PhoneInputForm`/`EmailInputForm` + `OtpVerifyForm` against the respective *attach* hooks (`use-attach-phone.ts`/`use-attach-email.ts`, never the public login OTP hooks), used by `profile/edit-profile-modal.tsx` (`onCancel` reverts the field's row to display mode). |
...
| `destinations/`, `payments/`, `wallet/`, `profile/`, `reviews/`, `notifications/` | per-feature. `profile/edit-profile-modal.tsx` (`EditProfileModal`) — the traveler profile's single "Edit Profile" button + `Modal`, replacing the old inline `edit-user-profile-form.tsx`/`verify-phone-cta.tsx` (both removed). Three independent field-edit affordances inside one modal: **Name** saves directly via `useUpdateProfile` (no OTP); **Email**/**Phone** each show a read-only row with a Change/Add/Verify button that swaps the row into `EmailVerificationFlow`/`PhoneVerificationFlow` respectively — the new value is only persisted once its OTP is verified, then the row reverts to display mode and `profileKeys.me()` is invalidated so `profile-header.tsx`'s verified badges flip without a refresh. This is an optional, account-level convenience, unrelated to the mandatory booking-contact flow above. |
