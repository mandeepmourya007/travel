...
CMD ["sh", "-c", "[ -f /etc/secrets/.env.prod ] && exec node --dns-result-order=ipv4first --env-file=/etc/secrets/.env.prod --import=tsx src/index.ts || exec node --dns-result-order=ipv4first --import=tsx src/index.ts"]
