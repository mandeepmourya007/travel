...
export function MediaTab() {
  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // For now, create object URLs as preview placeholders
      // In production, this would upload to Cloudinary using the signed URL flow
          ...
          {photoUrls.map((url: string, idx: number) => (
            <div key={idx} className="group relative aspect-video overflow-hidden rounded-lg bg-neutral-100">
              ...
              <button
                ...
                className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-neutral-900/50 text-white opacity-0 transition-opacity group-hover:opacity-100"
              >
