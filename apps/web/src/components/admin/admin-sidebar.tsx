...
export function AdminSidebar() {
  ...
  return (
    <>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 flex items-center border-t border-neutral-200 bg-white py-2 md:hidden">
        {MOBILE_NAV.map((item) => {
          const isActive = item.href === '/admin'
            ? pathname === '/admin'
            : pathname.startsWith(item.href)
          ...
          return (
            <Link
              ...
              prefetch={false}
              className={cn(
                'flex min-h-[44px] shrink-0 flex-col items-center justify-center gap-0.5 px-3 text-[10px] leading-tight',
