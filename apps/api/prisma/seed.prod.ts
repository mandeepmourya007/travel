...
async function main() {
  ...
  const goa = await prisma.destination.create({ data: { name: 'Goa', slug: 'goa', state: 'Goa', isPopular: true, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?w=800', description: 'Sun-kissed beaches, vibrant nightlife, and Portuguese heritage — Goa is India\'s ultimate coastal escape. From beach shacks in North Goa to serene backwaters in the south, every trip here is a vibe.' } })
  const manali = await prisma.destination.create({ data: { name: 'Manali', slug: 'manali', state: 'Himachal Pradesh', isPopular: true, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1571401835393-8c5f35328320?w=800', description: 'Snow-capped peaks, river rafting on the Beas, and the legendary Rohtang Pass — Manali is a year-round adventure hub in the heart of Himachal Pradesh.' } })
  const ladakh = await prisma.destination.create({ data: { name: 'Ladakh', slug: 'ladakh', state: 'Ladakh', isPopular: true, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?w=800', description: 'The land of high passes — Ladakh offers dramatic landscapes, ancient monasteries, and the iconic Pangong Lake. A bucket-list road trip for every adventure seeker.' } })
  const rishikesh = await prisma.destination.create({ data: { name: 'Rishikesh', slug: 'rishikesh', state: 'Uttarakhand', isPopular: true, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1482938289607-e9573fc25ebb?w=800', description: 'White-water rafting, bungee jumping, and riverside camping — Rishikesh is India\'s adventure capital, nestled in the foothills of the Himalayas along the sacred Ganges.' } })
  const jaipur = await prisma.destination.create({ data: { name: 'Jaipur', slug: 'jaipur', state: 'Rajasthan', isPopular: true, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1477587458883-47145ed94245?w=800', description: 'The Pink City — a royal blend of majestic forts, vibrant bazaars, and Rajasthani cuisine. Explore Amber Fort, Hawa Mahal, and the colorful streets of the old city.' } })
  const kasol = await prisma.destination.create({ data: { name: 'Kasol', slug: 'kasol', state: 'Himachal Pradesh', isPopular: true, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?w=800', description: 'A backpacker\'s paradise in the Parvati Valley — Kasol offers pine forest treks, riverside cafes, and the gateway to Kheerganga and Tosh.' } })
  const udaipur = await prisma.destination.create({ data: { name: 'Udaipur', slug: 'udaipur', state: 'Rajasthan', isPopular: true, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=800', description: 'The City of Lakes — Udaipur enchants with floating palaces, sunset boat rides on Lake Pichola, and romantic old-world Rajasthani charm.' } })
  const meghalaya = await prisma.destination.create({ data: { name: 'Meghalaya', slug: 'meghalaya', state: 'Meghalaya', isPopular: false, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1598091383021-15ddea10925d?w=800', description: 'Living root bridges, crystal-clear rivers, and the wettest place on Earth — Meghalaya is Northeast India\'s most stunning offbeat destination.' } })
  const hampi = await prisma.destination.create({ data: { name: 'Hampi', slug: 'hampi', state: 'Karnataka', isPopular: false, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1590050752117-238cb0fb12b1?w=800', description: 'A UNESCO World Heritage Site with surreal boulder landscapes, ancient Vijayanagara ruins, and a laid-back hippie vibe along the Tungabhadra River.' } })
  const lonavala = await prisma.destination.create({ data: { name: 'Lonavala', slug: 'lonavala', state: 'Maharashtra', isPopular: true, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1625505826533-5c80aca7d157?w=800', description: 'Mumbai and Pune\'s favourite weekend getaway — misty hills, monsoon waterfalls, and lush green valleys in the Western Ghats. Perfect for quick escapes.' } })
  const spiti = await prisma.destination.create({ data: { name: 'Spiti Valley', slug: 'spiti-valley', state: 'Himachal Pradesh', isPopular: false, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?w=800', description: 'A cold desert mountain valley — Spiti offers stark lunar landscapes, ancient Buddhist monasteries, and some of the most remote roads in India.' } })
  const coorg = await prisma.destination.create({ data: { name: 'Coorg', slug: 'coorg', state: 'Karnataka', isPopular: false, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800', description: 'The Scotland of India — Coorg is a coffee country paradise with misty hills, spice plantations, and Abbey Falls nestled in Karnataka\'s Western Ghats.' } })
  const varanasi = await prisma.destination.create({ data: { name: 'Varanasi', slug: 'varanasi', state: 'Uttar Pradesh', isPopular: false, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1561361513-2d000a50f0dc?w=800', description: 'One of the world\'s oldest living cities — Varanasi mesmerizes with its Ganga Aarti, ancient ghats, narrow lanes, and a spiritual energy unlike anywhere else.' } })
  const andaman = await prisma.destination.create({ data: { name: 'Andaman Islands', slug: 'andaman', state: 'Andaman & Nicobar', isPopular: false, tripCount: 0, photoUrl: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=800', description: 'Turquoise waters, pristine white-sand beaches, and world-class scuba diving — the Andaman Islands are India\'s tropical paradise in the Bay of Bengal.' } })
